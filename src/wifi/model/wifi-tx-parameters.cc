 ...
 */
 ...
 uint32_t
 WifiTxParameters::GetCurrentMpduNumber(Mac48Address receiver) const
 {
     NS_LOG_FUNCTION(this << receiver);
     ...
     if (infoIt == m_info.end())
     {
         NS_LOG_DEBUG("No information found for receiver: " << receiver);
         return 0;
     }
 
     auto seqNumbers = infoIt->second.seqNumbers;
 
     // 如果序列号信息为空，返回 0
     if (seqNumbers.empty())
     {
         NS_LOG_DEBUG("No sequence numbers found for receiver: " << receiver);
         return 0;
     }
 
     auto it = seqNumbers.find(0);
     if (it != seqNumbers.end())
     {
         size_t currentMpduNumber = it->second.size();
         NS_LOG_DEBUG("Number of MPDUs with TID=0: " << currentMpduNumber);
         return static_cast<uint32_t>(currentMpduNumber);
     }
     else
     {
         // 如果 TID = 0 的序列号不存在，返回 0
         NS_LOG_DEBUG("No MPDUs found with TID=0 for receiver: " << receiver);
         return 0;
     }
 }
