...
using namespace ns3;
std::string outputfile("PPDU.csv");
...
SaveParams(std::unordered_map<std::string, std::vector<uint32_t>> pm, std::vector<double> thpt, std::vector<double> p, std::vector<double> occ, std::vector<double> datarate, std::vector<double> blocktimerate, std::vector<double> blockrate, std::vector<uint32_t> blockCnt, std::vector<uint32_t> blockCnt_tr, std::vector<uint64_t> txopTime, std::vector<uint32_t> txopNum, std::vector<uint32_t> maxAmpduLength, std::vector<uint32_t> meanAmpduLength)
{
    ...
    std::cout << "No, Mode, CWmin1, CWmax1, CWmin2, CWmax2, Aifsn1, Aifsn2, TxopLimit1, TxopLimit2, RTS_CTS1, RTS_CTS2, MaxSlrc1, "
             ...
             "RedundancyFixedNumber2, BlockCnt1, BlockCnt2, BlockCnt1_True, BlockCnt2_True, TxopTime1(us), TxopTime2(us), TxopCnt1, TxopCnt2, MaxAmpduLength1, MaxAmpduLength2, p1, p2, Occupancy Rate 1, Occupancy Rate 2, datarate1, datarate2, Throughput(Mbps)" << std::endl;
    std::cout << params["No"][0] << ", " << 1 << ", " << 
                                    params["CWmins"][0] << ", " << params["CWmaxs"][0] << ", " <<
                                    params["CWmins"][1] << ", " << params["CWmaxs"][1] << ", " <<
                                    ...
                                    params["RedundancyFixedNumbers"][0] << ", " << params["RedundancyFixedNumbers"][1] << ", " <<
                                    res.blockCnt[0] << ", " <<  res.blockCnt[1] << ", " <<
                                    res.blockCnt_tr[0] << ", " <<  res.blockCnt_tr[1] << ", " <<
                                    res.txopTime[0] << ", " <<  res.txopTime[1] << ", " << res.txopNum[0]<< ", " << res.txopNum[1] << ", " << res.maxAmpduLength[0] << ", " << res.maxAmpduLength[1] << ", " <<
                                    res.p[0] << ", " << res.p[1] << ", " << res.occ[0] << ", " << res.occ[1] << ", " << res.datarate[0] << ", " << res.datarate[1] << ", " << res.throughput << std::endl;
                      ...
                      Time simulationTime)
{
    ...
    if (throughputQueue.size() == 2) {
        double tp = (throughputQueue.back() - throughputQueue.front()) * 8.0 / tputInterval.GetMicroSeconds();
    ...
    if (throughputQueue.size() == 1)
    {
        double tp = throughputQueue.front() * 8.0 / tputInterval.GetMicroSeconds();
        std::cout << "[" << (now - tputInterval).As(Time::S) << " - " << now.As(Time::S)
        << "] Throughput (Mbit/s):";
        std::cout << "\t\t" << tp << " ; "; // Mbit/s
        std::cout << std::endl;
...
void 
NotifyPpduTxDuration(Ptr<WifiPpdu const> ppdu, Time duration, uint8_t linkid)
{

    if(!ppdu->GetPsdu()->GetHeader(0).IsQosData()) return ;
    ...
    if(psdu->IsAggregate())
    {
        ...
        if(Simulator::Now().GetSeconds()<1.5)
        {
            ...
            if(nmpdus != 0) file <<"OBSS"<<","<< Simulator::Now().GetMicroSeconds() << "," << Simulator::Now().GetMicroSeconds() + duration.GetMicroSeconds() << "," << nmpdus << std::endl;
            file.close();
        }
    }
}
...
void 
NotifyPpduTxDurationmld(Ptr<WifiPpdu const> ppdu, Time duration, uint8_t linkid)
{
    if(!ppdu->GetPsdu()->GetHeader(0).IsQosData()) return ;
    Ptr<const WifiPsdu> psdu = ppdu->GetPsdu();
    if(psdu->IsAggregate())
    {
        ...
        if(Simulator::Now().GetSeconds()<1.5)
        {
            std::fstream file;
            file.open(outputfile, std::ios::out | std::ios::app);
            if(nmpdus != 0) file <<uint32_t(linkid)<<","<< Simulator::Now().GetMicroSeconds() << "," << Simulator::Now().GetMicroSeconds() + duration.GetMicroSeconds() << "," << nmpdus << std::endl;
            file.close();
        }
    }
}
...
void
SavetxopTime(std::unordered_map<uint8_t, std::vector<std::pair<uint64_t, uint64_t>>> txoplist,std::unordered_map<uint8_t, std::vector<uint32_t>> txopnums)
{
    std::fstream file;
    ...
    for(auto it = txoplist.begin(); it != txoplist.end(); it++)
    {
        ...
        {
            file << (uint32_t)linkid << "," << txopTime[i].first << "," << txopTime[i].second << std::endl;
        }
    }
    file.close();
}
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("jsonFile","json file",jsonfile);
    ...
    if(enable_interference&0x01){
        ...
        DynamicCast<WifiNetDevice>(apDev.Get(1))->GetPhy(0)->TraceConnectWithoutContext("PpduTxDuration",MakeCallback(&NotifyPpduTxDuration));
    }
    if(enable_interference&0x02){
        mac.SetType("ns3::ApWifiMac",
                    "BeaconInterval", TimeValue(MicroSeconds(beaconInterval)),
    ...
    positionAlloc->Add(Vector(0.0, 0.0, 0.0));
    positionAlloc->Add(Vector(0.0, 0.0, 0.0));
    positionAlloc->Add(Vector(0.0, 0.0, 1));
    positionAlloc->Add(Vector(0.0, 0.0, 1));
    positionAlloc->Add(Vector(0.0, 0.0, 1));
    ...
    if(enable_interference&0x02){
        ...
        client2.SetAttribute("Interval", TimeValue(Seconds(100)));
    ...
    client.SetAttribute("Interval", TimeValue(Seconds(packetInterval/2)));
    ...
    clientApp.Stop(simulationTime + Seconds(5.0));
    ...
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/Period", TimeValue(period/2));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/Mode", UintegerValue(mode));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/GridSearchEnable", BooleanValue(true));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/GridSearchParameter", StringValue(jsonfile));
    ...
    Config::ConnectWithoutContext("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/GetTxopTimeStats",MakeCallback(&SavetxopTime));
    std::fstream file;
    ...
    file.close();
    Simulator::Schedule(Seconds(1)+period/2, &PrintIntermediateTput, udp, dlserverApp, payloadSize, period/2, simulationTime + Seconds(5.0));
    
    uint64_t rx_totalbytesStart = 0;
    Simulator::Schedule(Seconds(5.0), &GetRxBytes2, udp, dlserverApp, payloadSize, std::ref(rx_totalbytesStart));

    Time statsEndTime = period * ((simulationTime + Seconds(5.0)) / period).GetInt();
    ...
    Simulator::Run();
    std::cout<<"SLD2 : "<<dlserverAppObss2.Get(0)->GetObject<UdpServer>()->GetReceived() << std::endl;
    std::cout << "Total DL Throughput [ +" << 5 <<" - " << statsEndTime.As(Time::S) << "]: \t\t\t" << (rx_totalbytes - rx_totalbytesStart) * 8.0 / (statsEndTime - Seconds(5.0)).GetMicroSeconds() << " Mbit/s" << std::endl;
    ...
    if (!results.empty()) 
        for (auto & res : results) {
            ...
            fout << params["No"][0] << ", " << (uint32_t)mode << ", " << 
                                    params["CWmins"][0] << ", " << params["CWmaxs"][0] << ", " <<
                                    params["CWmins"][1] << ", " << params["CWmaxs"][1] << ", " <<
                                    params["Aifsns"][0] << ", " << params["Aifsns"][1] << ", " <<
                                    params["TxopLimits"][0] << ", " << params["TxopLimits"][1] << ", " <<
                                    ...
                                    res.blockCnt_tr[0] << ", " <<  res.blockCnt_tr[1] << ", " <<
                                    res.txopTime[0] << ", " <<  res.txopTime[1] << ", " << res.txopNum[0]<< ", " << res.txopNum[1] << ", " << res.maxAmpduLength[0] << ", " << res.maxAmpduLength[1] << ", " <<
                                    res.p[0] << ", " << res.p[1] << ", " << res.occ[0] << ", " << res.occ[1] << ", " << res.datarate[0] << ", " << res.datarate[1] << ", " <<
                                    res.thpt[0] << ", " << res.thpt[1] << ", " << res.throughput << std::endl;
    }
    return 0;
}
