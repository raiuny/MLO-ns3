...
main(int argc, char* argv[])
{
    ...
    std::vector<int> mcs{11, 2};
    std::vector<int> bandwidth{40, 20};
    ...
    int16_t ampdu_maxn = 32;
    Config::Set("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/HeConfiguration/GuardInterval",
                    TimeValue(NanoSeconds(gi)));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_MaxAmsduSize",
            UintegerValue(0));
    Config::Set("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_MaxAmpduSize",
                    UintegerValue(128 * (payloadSize + 150)));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_MaxAmpduSize",
        UintegerValue(ampdu_maxn * 4 * (payloadSize + 150)));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/MpduBufferSize",
                            UintegerValue(mpduBufferSize));
    Config::Set("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/MpduBufferSize",
        UintegerValue(256));
    Config::Set("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/MpduBufferSize",
        UintegerValue(256));
    ...
    positionAlloc->Add(Vector(5.0, 0.0, 1));
    positionAlloc->Add(Vector(0.0, 10.0, 1));
    ...
    // AP 0
    const auto maxLoad2 = EhtPhy::GetDataRate(mcs[0], bandwidth[0] , NanoSeconds(gi), 1) * 2;
    const auto maxLoad5 = EhtPhy::GetDataRate(mcs[1], bandwidth[1] , NanoSeconds(gi), 1) * 2;
    ...
    Config::Set("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/MinCws", AttributeContainerValue<UintegerValue>(std::vector<int>{1}));
    Config::Set("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/MinCws", AttributeContainerValue<UintegerValue>(std::vector<int>{1}));
    Config::Set("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/MaxCws", AttributeContainerValue<UintegerValue>(std::vector<int>{15}));
    Config::Set("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/MaxCws", AttributeContainerValue<UintegerValue>(std::vector<int>{15}));
    Config::Set("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/MaxGroupSize", UintegerValue(maxGroupSize));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/Mode", UintegerValue(1));
    Config::Set("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/GridSearchEnable", BooleanValue(false));
    // Config::Set("/NodeList/3/DeviceList/*/$ns3::WifiNetDevice/Mac/BE_Txop/Mode", UintegerValue(1));
    ...
    flowMonitor1 = flowHelper.Install(apNodes.Get(0));
    ...
    Simulator::Schedule(Seconds(3.0), &PrintIntermediateTput, std::ref(dlCumulRxBytes_begin), std::ref(dlCumulRxBytes_end), true, dlserverApp, payloadSize, delta, tputInterval, simulationTime + Seconds(1.0));
    std::vector<uint64_t> rx_totalbytes(dlserverApp.GetN(), 0);
    Simulator::Schedule(simulationTime + Seconds(1.0), &GetRxBytes, true, dlserverApp, payloadSize, std::ref(rx_totalbytes));
    ...
    // apWifiDev->GetMac()->GetQosTxop(0)->GetMsduGrouper().print();
    ...
    //     if (t.protocol == 17) {
    //         std::cout << "Flow ID: " << flowId << " Source Address: " << t.sourceAddress << " Destination Address: " << t.destinationAddress << std::endl;
    //         std::cout << "Tx Packets: " << flowStats.txPackets << std::endl;
    //         std::cout << "Rx Packets: " << flowStats.rxPackets << std::endl;
    //         std::cout << "Throughput: " << flowStats.rxBytes * 8.0 / (simulationTime.GetMicroSeconds()) << " Mbps" << std::endl;
    //     }
    //  }
